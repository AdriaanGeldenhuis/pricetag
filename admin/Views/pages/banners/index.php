<div class="admin-page-header">
    ...
    <a href="<?php echo url('/admin/banners/create'); ?>" class="admin-btn admin-btn-primary">
...
<div class="admin-card mb-6">
    <div class="card-header flex items-center justify-between">
        <h2 class="font-semibold"><?php echo e($locationNames[$location] ?? ucwords(str_replace('_', ' ', $location))); ?></h2>
        <span class="text-sm text-muted"><?php echo count($locationBanners); ?> banner<?php echo count($locationBanners) !== 1 ? 's' : ''; ?></span>
    </div>
    <div class="card-body p-0">
        <div class="banner-list" data-location="<?php echo e($location); ?>">
            ...
            <div class="banner-item" data-id="<?php echo $banner['id']; ?>">
                ...
                <div class="banner-info flex-1">
                    <h4 class="font-medium"><?php echo e($banner['title'] ?: 'Untitled Banner'); ?></h4>
                    <?php if (!empty($banner['url'])): ?>
                    <p class="text-sm text-muted truncate"><?php echo e($banner['url']); ?></p>
                    <?php endif; ?>
                    <?php if (!empty($banner['starts_at']) || !empty($banner['expires_at'])): ?>
                    <p class="text-xs text-muted mt-1">
                        <?php if (!empty($banner['starts_at'])): ?>
                        Starts: <?php echo date('M j, Y', strtotime($banner['starts_at'])); ?>
                        <?php endif; ?>
                        <?php if (!empty($banner['expires_at'])): ?>
                        · Expires: <?php echo date('M j, Y', strtotime($banner['expires_at'])); ?>
                        <?php endif; ?>
                    </p>
                    <?php endif; ?>
                </div>
                <div class="banner-actions">
                    <button type="button"
                            onclick="toggleBanner(<?php echo $banner['id']; ?>, this)"
                            class="toggle-btn <?php echo $banner['is_active'] ? 'active' : ''; ?>"
                            title="<?php echo $banner['is_active'] ? 'Click to deactivate' : 'Click to activate'; ?>">
                        <span class="toggle-slider"></span>
                    </button>
                    <a href="<?php echo url('/admin/banners/' . $banner['id'] . '/edit'); ?>" class="admin-btn admin-btn-sm admin-btn-secondary">
                        Edit
                    </a>
                    <button type="button" onclick="deleteBanner(<?php echo $banner['id']; ?>)" class="admin-btn admin-btn-sm admin-btn-danger-outline">
...
function toggleBanner(id, btn) {
    fetch('<?php echo url('/admin/banners/'); ?>' + id + '/toggle', {
        method: 'POST',
        headers: {
...
function deleteBanner(id) {
    document.getElementById('deleteBannerForm').action = '<?php echo url('/admin/banners/'); ?>' + id;
...
document.querySelectorAll('.banner-list').forEach(function(list) {
    var draggedItem = null;

    list.querySelectorAll('.banner-item').forEach(function(item) {
...
function getDragAfterElement(container, y) {
    var draggableElements = Array.from(container.querySelectorAll('.banner-item:not(.dragging)'));
...
function saveBannerOrder(list) {
    ...
    fetch('<?php echo url('/admin/banners/reorder'); ?>', {
        method: 'POST',
        headers: {
            'X-CSRF-Token': '<?php echo csrf_token(); ?>',
